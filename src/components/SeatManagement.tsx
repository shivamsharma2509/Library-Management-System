...
const SeatManagement: React.FC<SeatManagementProps> = ({
  ...
  const handleSeatClick = (seat: Seat) => {
    if (seat.isOccupied) {
      // Show seat details or release option
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Seat Grid */}
      <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Seat Layout</h3>
        <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-2">
          {seats.map((seat) => (
            <button
              key={seat.number}
              onClick={() => handleSeatClick(seat)}
              className={`aspect-square rounded-lg border-2 flex flex-col items-center justify-center p-2 text-xs font-medium transition-all hover:scale-105 ${
                seat.isOccupied
      ...
      {showAssignModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold mb-4">Assign Seat {selectedSeat}</h3>
            ...
            <div className="max-h-60 overflow-y-auto space-y-2">
              {availableStudents.length > 0 ? (
                ...
                <p className="text-gray-500 text-center py-4">No available students found</p>
              )}
            </div>
      ...
      {/* Seat Details Modal */}
      {selectedSeat && !showAssignModal && seats.find(s => s.number === selectedSeat)?.isOccupied && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            <h3 className="text-lg font-semibold mb-4">Seat {selectedSeat} Details</h3>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900">
                      {seat?.assignedDate ? new Date(seat.assignedDate).toLocaleDateString('en-IN') : 'N/A'}
                    </p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700">Fee Expiry</label>
                    <p className="text-gray-900">{new Date(student.feeExpiryDate).toLocaleDateString('en-IN')}</p>
                  </div>
                </div>
              ) : null;
            })()}
